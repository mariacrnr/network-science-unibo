...
spread = 0.01; % Weight spreading ratio
...
for t = 1:iterations
    ...
    % Disease Spread
    transmissionProb = beta * (transmissionM' * currentInfected) .* (~immuneNodes);
    newlyInfected = double(rand(N, 1) <= transmissionProb);
    
    % Update Infected Individuals
    currentInfected = currentInfected | (currentSusceptible & newlyInfected);
    
    % Apply Recovery
    recovered = rand(N, 1) <= delta;
    immuneNodes = immuneNodes | recovered;

    currentInfected = currentInfected & ~recovered;
    currentSusceptible = ~currentInfected & ~immuneNodes;

    % Vary weight strength according to infection status
    adjacencyM =  adjacencyM + spread * adjacencyM .* currentInfected; % Increase weights for infected nodes
    adjacencyM =  adjacencyM - spread * adjacencyM .* currentSusceptible; % Decrease weights for recovered nodes
    adjacencyM = max(0, min(1, adjacencyM));  % Ensure the updated weights stay within the [0, 1] range

    % Record infection levels
    infectionHistory(t) = sum(currentInfected);
    immuneHistory(t) = sum(immuneNodes);
...
plot(1:iterations, immuneHistory, 'b', 'LineWidth', 2, 'DisplayName', 'Immune');
hold off;
